...
# Función para obtener el último periodo
def get_ultimo_periodo_agua(hogar_id):
    db, c = get_db()
...
# Función para obtener el consumo de un período específico
def get_consumo_periodo_agua(hogar_id, periodo_id, columna):
    db, c = get_db()
    
    try:
        query = f"""
        SELECT ca.{columna}, p.inicio
        FROM consumo_agua ca
        JOIN periodos p ON ca.periodo_id = p.id
        WHERE ca.hogar_id = %s AND ca.periodo_id = %s
        """
        c.execute(query, (hogar_id, periodo_id))
        resultado = c.fetchone()
...
# Integración para obtener y calcular los consumos
def get_calcular_cambio_agua(hogar_id, columna):
    db, c = get_db()
    try:
...
def user_domotica():
    ...
    datos_grafica_agua_litros, diferencia_porcentual_litros = get_consumo_agua('consumo_litros')
    ...
    # Obtener último periodo y cambios porcentuales de agua para precio
    consumo_actual_agua_mxn, cambio_porcentual_agua_mxn, inicio_anterior_agua_mxn = get_calcular_cambio_agua(hogar_id, 'precio_total')
    
    # Formatear fechas al mes en español
    inicio_anterior_litros_formateado = formatear_fecha(inicio_anterior_litros)
    inicio_anterior_agua_mxn_formateado = formatear_fecha(inicio_anterior_agua_mxn)
    ...
    return render_template(
        ...
        consumo_actual_litros=consumo_actual_litros,
        cambio_porcentual_litros=cambio_porcentual_litros,
        ...
        diferencia_porcentual_agua_mxn=diferencia_porcentual_agua_mxn,
        # GRAFICA PIE - TIPOS DE DISPOSITIVOS
